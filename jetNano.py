...
DIR_PIN = 20  # Pin for Direction
STEP_PIN = 21  # Pin for Step
DIR = 1  # Direction (1 for forward, 0 for backward)
STEPS = 1000  # Number of steps
DELAY = 0.001  # Delay between steps in seconds (1ms)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    # Write to CSV
                    if save_csv:
                        write_to_csv(p.name, names[int(cls)], float(conf))
                ...
                # Trigger motor movement if an object is detected
                move_motor(STEPS, DELAY)
